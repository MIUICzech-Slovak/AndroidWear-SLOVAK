...
path=$up/RRO-2.0-SK/WristGesturesTutorial.apk
...
$aapt package -f -M $path/AndroidManifest.xml -S $path/res/ -I $up/tools/android.jar -F $up/output/WristGesturesTutorial-overlay.apk

echo "Signing overlay.apk";

java -Xmx4048m -jar $up/tools/apksigner.jar sign --key $key_pk8 --cert $key_pem $up/output/WristGesturesTutorial-overlay.apk			
